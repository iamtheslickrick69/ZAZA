      ...
      >
        <div className="max-w-screen-xl mx-auto px-4 lg:px-8">
          <div className="flex flex-col items-center">
            {companyLogo && (
              <div className="mb-6 relative h-8 w-32">
                <Image
                  src={companyLogo}
                  alt="Company logo"
                  fill
            ...
            <p
              className="max-w-md text-balance text-center text-lg sm:text-xl text-foreground leading-relaxed"
              dangerouslySetInnerHTML={{ __html: `"${formattedQuote}"` }}
            />
            <h5 className="mt-5 font-medium text-muted-foreground">
              {authorName}
            </h5>
            <h5 className="mt-1 text-sm text-foreground/40">
              {authorPosition}
            </h5>
            {authorImage && (
              <div className="mt-4 relative size-12 rounded-full overflow-hidden bg-muted">
                <Image
                  src={authorImage}
                  alt={authorName}
                  fill
