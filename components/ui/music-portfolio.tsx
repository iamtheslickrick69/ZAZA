...
// Try to register ScrambleTextPlugin if available (it's a paid GSAP plugin)
...
if (typeof window !== 'undefined') {
  try {
    ...
    // ScrambleTextPlugin not available - will use fallback
  }
}
...
const ProjectItem = forwardRef<HTMLLIElement, ProjectItemProps>(
  ({ project, index, onMouseEnter, onMouseLeave, isActive, isIdle }, ref) => {
    ...
    useEffect(() => {
      if (isActive && ScrambleTextPlugin) {
    ...
    return (
      ...
      >
        <span className="project-data index">{String(index + 1).padStart(2, '0')}</span>
...
const MusicPortfolio: React.FC<MusicPortfolioProps> = ({
  ...
  const coordinates = LOCATION.display !== false
...
export { MusicPortfolio };
export type { Project, MusicPortfolioProps };
