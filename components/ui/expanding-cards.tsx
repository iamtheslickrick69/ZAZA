      ...
      {items.map((item, index) => {
        ...
        return (
          <li
            key={item.id}
            className={cn(
              "group relative cursor-pointer overflow-hidden rounded-xl border bg-card text-card-foreground shadow-sm",
              "md:min-w-[80px]",
              "min-h-0 min-w-0",
          ...
          >
            ...
            <article className="absolute inset-0 flex flex-col justify-end gap-2 p-5">
              <h3 className="hidden origin-left rotate-90 text-xs font-medium uppercase tracking-widest text-white/70 opacity-100 transition-all duration-300 ease-out md:block group-data-[active=true]:opacity-0">
                {item.title}
              </h3>
